    ...
    function renderLocations(filter = 'All') {
        ...
        locations.forEach((location, index) => {
            if (filter === 'All' || location.category === filter) {
                ...
                marker.bindPopup(`<b>${location.name}</b><br>${location.description}<br><button class="add-to-itinerary" data-index="${index}">Add to Itinerary</button>`);
                ...
                card.innerHTML = `
                    ...
                    <button class="add-to-itinerary bg-blue-600 text-white px-3 py-1 rounded-full text-sm hover:bg-blue-700" data-index="${index}">Add to Itinerary</button>
                `;
